...
As with vectors, our exposition is only the tip of the iceberg  
...
Note that multiplication of scalars is commutative since for two scalars (numbers), $a$ and $b$, we always have that $ab = ba$. With matrices, this is not the case. For example, $\tilde{C}=BA$ yields a different result from $C= AB$. As the reader may verify,
%
\begin{equation}
